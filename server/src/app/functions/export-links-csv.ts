...
export async function exportLinks(
  ...
  const { sql, params } = db
    .select({
      id: schema.links.id,
      original_link: schema.links.originalLink,
      short_link: schema.links.shortLink,
    ...
    .where(
      searchQuery
        ? ilike(schema.links.originalLink, `%${searchQuery}%`)
        : undefined
    )
    .toSQL()
  ...
  const csv = stringify({
    ...
    columns: [
      ...
      { key: 'short_link', header: 'Shortened URL' },
